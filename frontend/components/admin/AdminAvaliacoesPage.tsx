    ...
    <div className="flex w-full flex-col items-start">
      ...
      <div className="grid w-full grid-cols-3 gap-4 pt-6">
        <div className="rounded-[12px] border border-[#e6e4df] bg-white p-5">
          <p className="text-sm font-semibold text-[#17181d]">Distribuição de notas</p>
        ...
        <div className="col-span-2 rounded-[12px] border border-[#e6e4df] bg-white p-5">
          <p className="text-sm font-semibold text-[#17181d]">Por profissional</p>
            ...
            {byBarber.map((b) => (
              <div key={b.name}>
                <div className="flex items-center gap-3">
                  ...
                  <p className="flex-1 text-sm font-medium text-[#17181d]">{b.name}</p>
                ...
                <div className="mt-2 ml-11 h-1.5 overflow-hidden rounded-full bg-[#f0efea]">
                  <div className="h-full rounded-full bg-[#6c4cf1]" style={{ width: `${b.percent}%` }} />
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="w-full pt-8">
        <h2 className="text-lg font-semibold text-[#17181d]">Avaliações recentes</h2>
        <div className="flex flex-col gap-4 pt-4">
          {reviews.map((review) => (
            <div key={`${review.name}-${review.date}`} className="rounded-[12px] border border-[#e6e4df] bg-white p-5">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-[#17181d]">{review.name}</p>
              ...
              <p className="pt-3 text-sm text-[#17181d]">{review.text}</p>

              {review.reply ? (
                <div className="mt-4 rounded-[10px] border-l-2 border-[#6c4cf1] bg-[#ede9fd] p-4">
                  <p className="text-xs font-semibold text-[#6c4cf1]">Resposta da barbearia</p>
                  <p className="pt-1 text-sm text-[#17181d]">{review.reply}</p>
                </div>
              ) : (
                <button
                  type="button"
                  className="mt-4 flex items-center gap-2 rounded-[8px] border border-[#e6e4df] bg-white px-3 py-1.5 text-sm font-medium text-[#17181d] transition hover:bg-[#f7f6f2]"
                >
